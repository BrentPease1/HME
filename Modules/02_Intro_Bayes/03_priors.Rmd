...
Hence the name, "flat, uninformative priors"
...
```{r warning = F}
beta_prior <- data.frame(x = seq(0,1,0.01),
                      y = dbeta(seq(0,1,0.01), 1, 1),
                      term = 'prior')
binom_lik <- data.frame(x = seq(0,1,length.out = 11),
                        y = dbinom(0:10, size = 10, prob = 0.2),
                        term = 'likelihood')

beta_post <- data.frame(x = seq(0,1,0.001),
                      y = dbeta(seq(0,1,0.001), 3, 9),
                      term = 'posterior')
beta_out <- rbind(beta_prior, binom_lik)
beta_out <- rbind(beta_out, beta_post)
ggplot() + 
  geom_area(data = beta_out %>% filter(term == 'prior'), mapping = aes(x, y), fill = pal[3], color = pal[3]) + 
    geom_path(data = beta_out %>% filter(term == 'likelihood'), mapping = aes(x, y), linesize = 1, col = pal[5]) +
  geom_path(data = beta_out %>% filter(term == 'posterior'), mapping = aes(x, y), linesize = 1, col = pal[2])
...
If $y \sim Binomial(N, p)$ and the only data we have in $y$ (we don't know $N$), we cannot estimate $N$ and $p$ no matter how much data we have**^** 
